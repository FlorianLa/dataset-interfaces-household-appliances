...
def constrToggle(im0, line_thickness, names, label, xyxy, seen, toggle):
    ...
    if x > y:
        z = x
        side = ['right', 'left']               # right or above
    else:
        z = y
        side = ['above', 'under']                # left or under
    ...
    if toggle == 1:                 # blauer Pfeil rechts, gelber Dreieckspitzen
        # border
        ...
        borderWidth = 12            # width in pxl of border (if box filled, half of border is inside)
    ...
    elif toggle == 2:               # hellgrüner Pfeil links mit dünnen Spitzen, dunkelgrüne Border
        # border
        labelVal = ''               # label[:-5] or label
        styleLine = 'line'          # 'dotted', 'dashed' or 'line'
        gab = 100                   # gab for dotted / dashed
        colorBorder = (0, 50, 0)    # color of the border BGR
        ...
        dis = 0                     # Distance of arrow to border (recommended: 0, d1 if 'left' or 'right', d2 if 'above' or 'under')
        color = (0, 255, 0)         # color of arrow
        aboveright = 'line'         # right or upper arrowhead ('line', 'triangle', 'none')
        underleft = 'line'          # under or left arrowhead ('line', 'triangle', 'none')
        colorAboveR = (0, 255, 0)   # color of right or upper arrowhead
        colorUnderL = (0, 255, 0)   # color arrowhead under left
        width = 30                  # width in pxl of arrow
        aboveRWidth = 30            # width in pxl of upper or right arrowhead (only if arrowhead == 'line')
        underLWidth = 30            # width in pxl of under or left arrowhead (only if arrowhead == 'line')
